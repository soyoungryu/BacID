...
fdr <- p.adjust(pvalue, method="fdr") #please remove decoy results after running it

sum(fdr[correct>=0]<=0.01) #Among non-decoy spectra (correct=0 or 1), the number of identificaitons with fdr <= 0.01 #175
sum(correct[fdr<=0.01]==0)/sum(fdr[correct>=0]<=0.01) #true FDR estimation #0.005714286
