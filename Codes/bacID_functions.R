...
#mz.var0: mz.var0 where peak m/z variance is mz.var0 times a reference m/z
#int.var: peak intensity variance
...
matching_bi <- function(poly, ref,   true.id=NULL, mass.tolerance=2000,
						mz.var0=0.0028, int.var=0.3604, ini.gamma=0.50,
						ini.alpha=NULL, esp=0.0001, max.iter=100){
	m=2
	...
	loglik.list = prob.list =  array(c(NA), dim=ncol(db))
		...
		res <- fit_model_C2(poly, monos, mass.tolerance=mass.tolerance,
				mz.var0=mz.var0, int.var=int.var, ini.gamma=ini.gamma,
...
conf_score <- function(poly, ref, true.id, m=100, p=0.70, mass.tolerance=2000,
	...
	correct.id1 <- ifelse(id1 %in% true.id, 1, ifelse(substr(id1, 1,5)=='decoy',-1,0))	
	correct.id2 <- ifelse(id2 %in% true.id, 1, ifelse(substr(id2, 1,5)=='decoy',-1,0))		
	return(list(id=c(id1, id2), correct.id=c(correct.id1,correct.id2), scores=c(score1, score2)))
}
...
e_step <- function(d.mz, d.int, status, param,  int.var){
	e.q = rep(NA, length(status))
	e.q[status==1]=1
	e.q[status==2]=0
	if (sum(status==3)>0){
	num = param$gamma*f(d.mz[[1]][status==3], d.int[[1]][status==3], param$alpha[1], int.var)
	den = param$gamma*f(d.mz[[1]][status==3], d.int[[1]][status==3], param$alpha[1], int.var)+(1-param$gamma)*f(d.mz[[2]][status==3], d.int[[2]][status==3], param$alpha[2], int.var)
	e.q[status==3]=(num/den)
	}
	return(e.q)
}

f <- function(d.mz, d.int, alpha, int.var){
	res=dnorm(d.mz, mean=0, sd=1)*dnorm(d.int, mean=alpha/sqrt(int.var), sd=1)
	return(res)
}
m_step <- function(d.mz, d.int, status, e.q, int.var){
	gamma <- sum(e.q[status==3])/sum(status==3)	
	alpha <- c()
	alpha[1] <- ifelse((sum(e.q[status==3])+sum(status==1))>0,sqrt(int.var)*(sum(e.q[status==3]*d.int[[1]][status==3])+sum(d.int[[1]][status==1]))/(sum(e.q[status==3])+sum(status==1)),0)
	alpha[2] <- ifelse((sum(1-e.q[status==3])+sum(status==2))>0, sqrt(int.var)*(sum((1-e.q[status==3])*d.int[[2]][status==3])+sum(d.int[[2]][status==2]))/(sum(1-e.q[status==3])+sum(status==2)),0)
	return(list(gamma=gamma, alpha=alpha))
}
loglik <- function(d.mz, d.int, status, param, r.int, r.mz, int.var){
	log.lik = -(log(r.mz)+log(r.int))*sum(status==0)+
	sum(log(param$gamma*f(d.mz[[1]][status==3], d.int[[1]][status==3], param$alpha[1], int.var)+(1-param$gamma)*f(d.mz[[2]][status==3], d.int[[2]][status==3], param$alpha[2], int.var)))+
	sum(log(f(d.mz[[1]][status==1], d.int[[1]][status==1], param$alpha[1], int.var)))+
	sum(log(f(d.mz[[2]][status==2], d.int[[2]][status==2], param$alpha[2], int.var)))
	return(log.lik)
}
...
###############
min.count=3
r.mz <- (12000-4000)/sqrt(4000*mz.var0)
r.int <- max(poly$int)/sqrt(int.var)
...
for (k in 1:2){
	d.mz[[k]]=(poly$mz-monos[[k]]$mz)/sqrt(monos[[k]]$mz*mz.var0)
	d.int[[k]]=(poly$int-monos[[k]]$int)/sqrt(int.var)
}
param <- list(); 
...
j <- 0
	while(to.run){
		j <- j+1				
		e.q <- e_step(d.mz, d.int, status, param,  int.var)
		if (j>1) {param.old <- param}
		param <- m_step(d.mz, d.int, status, e.q,  int.var)
		if (j>1) {l.old <- l.new}
		l.new <- loglik(d.mz, d.int, status, param, r.int, r.mz, int.var)
...
normalize.intensity = function(data, log.transform=FALSE, intensity = 100, constant = 1) {
  new.data = data
